...
#include <string_view>
#include "tscore/ts_HashContext.h"
...
/// Crypto hash output.
using CryptoHashData = CacheHashContext::value_type;

union CryptoHashData {
  ...
  CryptoHashData() { memset(this, 0, sizeof(*this)); }

  /// Assignment - bitwise copy.
  CryptoHashData &
  operator=(CryptoHashData const &that)
  {
    if (this != &that) {
  ...
  bool
  operator==(CryptoHashData const &that) const
  {
...
extern CryptoHashData const CRYPTO_HASH_ZERO;
